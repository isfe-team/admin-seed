  ...
  <div class="pq-table-wrapper">
    ...
    >
      ...
      <template slot-scope="text" slot="ellipsis-with-tooltip">
        <ATooltip v-if="text && text.length >= 2" placement="topLeft" class="pq-table-tooltip" :title="text">{{ text }}</ATooltip>
...
export default {
  ...
  methods: {
    ...
    handelChangeTable (pagination, filters, sorter) {
      this.$emit('table-change', sorter, filters)
    },
    // 重置分页数据
    resetPagination () {
      this.pagination.currentPage = 1
    ...
    emitOperation (operation, record) {
      ...
      const disabled = !!(typeof operation.disabled === 'function'
    ...
    loadData () {
      return this.loadDataApi(this.pagination.currentPage, this.pagination.pageSize)
        .then((data) => {
          const pageData = this.getDataPageData(data)
          if (data) {
            const totalSize = this.getTotalSize(data)
            this.pagination.totalRows = totalSize
            // 解决删除最后一页的最后一条数据时，没有触发change事件重新load的问题
            const maxPage = Math.ceil(totalSize / this.pagination.pageSize)
            if (maxPage > 0 && maxPage < this.pagination.currentPage) {
              return this.handleCurrentChange(maxPage)
            }
          }
          const listData = this.transformListData(pageData)
...
<style scoped lang="less">
  ...
  @media screen and (max-width: 1400px) {
    .pq-table-wrapper .pq-table /deep/ table {
      font-size: 12px;
    }
  }
</style>
